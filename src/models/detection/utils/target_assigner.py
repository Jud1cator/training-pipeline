...
class TargetAssigner(object):
    ...
    def assign(
            self, anchors: BoxList, groundtruth_boxes: BoxList, groundtruth_labels=None,
        ...
        if groundtruth_labels is None:
            groundtruth_labels = torch.ones(groundtruth_boxes.num_boxes(),
    ...
    def _create_regression_targets(
        ...
        unmatched_ignored_reg_targets = self._default_regression_target(
            device
        ).repeat(match.match_results.shape[0], 1)

        matched_anchors_mask = match.matched_column_indicator()
        reg_targets = torch.where(
            matched_anchors_mask.unsqueeze(
                1
