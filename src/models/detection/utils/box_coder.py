...
def batch_decode(encoded_boxes, box_coder: FasterRcnnBoxCoder, anchors: BoxList):
    ...
    if encoded_boxes.shape[1] != anchors.num_boxes():
        raise ValueError(
            f'The number of anchors inferred from encoded_boxes and anchors are inconsistent:'
            f' shape[1] of encoded_boxes {encoded_boxes.shape[1]} should be equal to the number'
            f' of anchors: {anchors.num_boxes()}.'
        )
